...
3. 在做Johnson距离子算法时，可能会由于反复的浮点数计算导致所有子集中$\Delta(Y_s) \gt 0, \Delta_i(Y_s) \gt 0,\Delta_j(Y_s \cup \{y_j\}) < 0$的判断失效，从而导致算法异常退出（这也是Gino所说的ill-condition：当Simplex退化成平面时的情况）。原始论文解决的方法是采用一个兜底程序（Backup Procedure）：暴力遍历所有子集，找到满足$\Delta(Y_s) \gt 0, \Delta_i(Y_s) \gt 0$的距离原点的最近点作为返回值。这总是可以找到的（比如只含有一个点的集合）。这个方法叫Johnson Robust。
...
GinoGJK中就提出了帧间一致性：当Polytope使用爬山法构造时（爬山法详见上一篇文章），可以利用上一帧最后找支撑点的方向作为下一帧的开始方向查找，一般会大大减少爬山法搜索的时长。
