(async () => {
  ...
  function snapshot() {
    return {
      queue: queue.map(item => ({ r: item.r, c: item.c, dist: item.dist })),
  ...
  function doDijkstraStep() {
    ...
    for (const [dr, dc] of dirs) {
      ...
      const newDist = item.dist + cell.cost;
  ...
  function finish() {
    ...
    while (queue.length > 0) {
      ...
      const dirs = [[-1, 0], [0, 1], [1, 0], [0, -1]];
      for (const [dr, dc] of dirs) {
        const nr = item.r + dr, nc = item.c + dc;
        if (nr < 0 || nr >= ROWS || nc < 0 || nc >= COLS) continue;
        const cell = grid[nr][nc];
        if (cell.cost === Infinity) continue;
        const newDist = item.dist + cell.cost;
  ...
  function setEndPoint(r, c) {
    stopAutoPlay();
    if (grid[r][c].cost === Infinity) return;
    endR = r;
    endC = c;
    rebuildPath();
    render();
  ...
  const container = document.getElementById('pixi-dijkstra-demo');
  ...
  function render() {
    ...
    if (startR >= 0) {
      ...
      if (autoPlaying) {
        ...
        if (queue.length > 0) buttons.push({ l: 'Next', c: 0x1a73e8, a: dijkstraStep });
